...
function generatePersonaName($stats) {
    // 1. Determine Gender Prefix
    $genderPrefix = "Shopper";
    if ($stats['dominant_gender'] === 'Male') {
        $genderPrefix = "Men's";
    } elseif ($stats['dominant_gender'] === 'Female') {
        $genderPrefix = "Women's";
    }

    // 2. Determine Income Descriptor 
    ...
    // 3. Construct Base Name
    if ($genderPrefix === "Shopper") {
        $name = "$incomeDesc Shoppers";
    } else {
        $name = "$genderPrefix $incomeDesc Segment";
    }

    // 4. Append Regional Context (if known)
    if (!empty($stats['dominant_region']) && $stats['dominant_region'] !== 'Unknown') {
        $name .= " ({$stats['dominant_region']})";
    }
...
function updateDatabase($pdo, $labels, $clusterStats) {
    try {
        ...
        $pdo->rollBack(); // Critical: Revert changes
        
        // LOG: Critical failure in background job
        error_log("[Clustering Job Failed] " . $e->getMessage());
        
        // CLI Feedback (if running in terminal)
        if (php_sapi_name() === 'cli') {
            echo "Error: Operation failed. Check logs for details.\n";
        }
        exit(1); // Return non-zero exit code for system monitors
    }
}
...
echo str_repeat("=", 70) . "\n";
echo "CLUSTERING SUMMARY (NAMING COMPARISON)\n";
echo str_repeat("=", 70) . "\n\n";

foreach ($clusterStats as $stats) {
    // Generate OLD name
    $oldName = generateClusterName($stats['avg_age'], $stats['avg_income'], $stats['avg_purchase_amount']);
    
    // Generate NEW name
    $newName = generatePersonaName($stats);

    echo "Cluster {$stats['cluster_id']}:\n";
    echo "  [Old System]: $oldName\n"; // e.g., High-Income Middle-Aged Premium
    echo "  [New System]: $newName\n"; // e.g., Women's Elite Segment (East Coast)
    echo " --------------------------------------------------------\n";
